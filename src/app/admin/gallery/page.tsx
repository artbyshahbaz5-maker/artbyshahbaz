...
export default function AdminGalleryPage() {
  ...
  async function load() {
    ...
    if (!res.success) alert(res.message || "Failed to load gallery.");
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch(
        isEdit ? `/api/admin/gallery/${editingId}` : "/api/admin/gallery",
        {
      ...
      if (!res.success) {
        alert(res.message || "Failed to save gallery photo.");
        return;
      }
      setOpen(false);
      setEditingId(null);
  ...
  async function handleDelete(id: string) {
    ...
    const res = await fetch(`/api/admin/gallery/${id}`, { method: "DELETE" }).then((r) => r.json());
          ...
          {gallery.map((item) => (
            <div key={item.id} className="relative group rounded-xl overflow-hidden border border-neutral-800">
              ...
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                <Button size="sm" variant="adminSecondary" onClick={() => openEdit(item)} className="gap-1.5 text-xs">
          ...
          <div className="space-y-5 mt-2">
            ...
            <div>
              ...
              <Button type="button" variant="adminSecondary" onClick={() => fileRef.current?.click()} disabled={uploading} className="w-full gap-2">
                ...
                {uploading ? "Uploading..." : imageUrl ? "Replace Photo" : "Choose & Upload Photo"}
              </Button>
            </div>
            ...
            <div className="flex gap-3">
              <Button onClick={handleSave} disabled={saving || !imageUrl} className="flex-1 bg-gold-500 hover:bg-gold-400 text-neutral-950 font-semibold">
                {saving ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : null} {isEdit ? "Save Changes" : "Add to Gallery"}
              </Button>
