...
const EMPTY: Partial<FAQ> = { question: "", answer: "", is_visible: true };
...
export default function AdminFaqsPage() {
  ...
  async function toggleVisible(f: FAQ) {
    const res = await fetch(`/api/admin/faqs/${f.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ is_visible: !(f.is_visible ?? true) }),
    }).then((r) => r.json());
    if (!res.success) alert(res.message || "Failed to update FAQ.");
    load();
  }
    ...
    <div>
      ...
      {loading ? <div className="flex items-center gap-2 text-neutral-400 py-10"><Loader2 className="h-5 w-5 animate-spin" /> Loading...</div> : (
        <div className="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-3">
          {faqs.length === 0 && <p className="text-neutral-500 text-sm py-6 col-span-full">No FAQs yet.</p>}
          {faqs.map((f, i) => (
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {!(f.is_visible ?? true) && <Badge variant="secondary" className="text-[10px] flex-shrink-0">Hidden</Badge>}
                  </div>
                  <p className="text-neutral-400 text-sm leading-relaxed">{f.answer}</p>
                </div>
                <div className="flex gap-1.5 flex-shrink-0">
                  <Button size="sm" variant="ghost" onClick={() => toggleVisible(f)} className="h-7 px-2 text-[11px] text-neutral-400 hover:text-white hover:bg-neutral-800">
                    {f.is_visible ?? true ? "Hide" : "Show"}
                  </Button>
        ...
        <DialogContent className="max-w-lg bg-neutral-900 border-neutral-700 text-white">
          ...
          <div className="space-y-4 mt-2">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={editing.is_visible ?? true} onChange={(e) => setEditing((p) => ({ ...p, is_visible: e.target.checked }))} className="h-4 w-4 rounded accent-gold-500" />
