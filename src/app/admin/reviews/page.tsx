...
export default function AdminReviewsPage() {
  ...
  async function load() {
    ...
    if (!res.success) alert(res.message || "Failed to load reviews.");
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch(
        isEdit ? `/api/admin/reviews/${editingId}` : "/api/admin/reviews",
        {
  ...
  async function toggleVisible(r: Review) {
    const res = await fetch(`/api/admin/reviews/${r.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ is_visible: !(r.is_visible ?? true) }),
    }).then((res) => res.json());
    if (!res.success) alert(res.message || "Failed to update review.");
    load();
  }

  async function handleDelete(id: string) {
    if (!confirm("Delete this review?")) return;
    const res = await fetch(`/api/admin/reviews/${id}`, { method: "DELETE" }).then((r) => r.json());
    ...
    <div>
          ...
          {reviews.map((r) => (
            <div key={r.id} className="bg-neutral-900 border border-neutral-800 rounded-xl p-5 hover:border-neutral-700 transition-colors">
              <div className="flex items-start justify-between mb-3">
                ...
                {!(r.is_visible ?? true) && <Badge variant="secondary" className="text-[10px]">Hidden</Badge>}
              </div>
              ...
              <div className="flex gap-2 mt-4">
                <Button size="sm" variant="adminSecondary" onClick={() => openEdit(r)} className="flex-1 text-xs gap-1.5">
                  <Pencil className="h-3 w-3" /> Edit
                </Button>
                <Button size="sm" variant="adminSecondary" onClick={() => toggleVisible(r)} className="text-xs">
                  {r.is_visible ?? true ? "Hide" : "Show"}
                </Button>
                <Button size="sm" variant="adminSecondary" onClick={() => handleDelete(r.id)} className="border-red-800/60 text-red-400 hover:text-red-300 hover:bg-red-900/20 text-xs">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md bg-neutral-900 border-neutral-700 text-white">
          <DialogHeader>
            <DialogTitle className="font-serif text-xl text-white">{isEdit ? "Edit Review" : "Add Review"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.is_visible} onChange={(e) => setForm((p) => ({ ...p, is_visible: e.target.checked }))} className="h-4 w-4 rounded accent-gold-500" />
            ...
            <div className="flex gap-3 pt-2">
              <Button onClick={handleSave} disabled={saving || !form.client_name || !form.review_text} className="flex-1 bg-gold-500 hover:bg-gold-400 text-neutral-950 font-semibold">
                {saving ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : null} {isEdit ? "Save Changes" : "Add Review"}
              </Button>
