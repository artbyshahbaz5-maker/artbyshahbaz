...
export async function POST(req: NextRequest) {
  ...
  const { question, answer, sort_order, is_visible } = body;
  if (!question || !answer) return NextResponse.json({ success: false, message: "question and answer required." }, { status: 400 });
  const { data, error } = await supabase.from("faqs").insert([{ question, answer, sort_order: sort_order ?? 0, is_visible: is_visible ?? true }]).select().single();
  if (error) return NextResponse.json({ success: false, message: error.message }, { status: 500 });
