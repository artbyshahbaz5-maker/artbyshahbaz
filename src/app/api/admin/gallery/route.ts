...
export async function POST(req: NextRequest) {
  ...
  const { image_url, title, description, category, sort_order } = body;
  if (!image_url) return NextResponse.json({ success: false, message: "image_url is required." }, { status: 400 });
  const { data, error } = await supabase.from("gallery").insert([{ image_url, title: title || "", description: description || "", category: category || "Bridal", sort_order: sort_order ?? 0 }]).select().single();
  if (error) return NextResponse.json({ success: false, message: error.message }, { status: 500 });
